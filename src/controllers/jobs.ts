...
import JobCategory from "../database/jobCategory.model";
...
export const createJob = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    const mailOptions = {
      ...
      to: interestedUsers,
      subject: "POSTING OF NEW JOB",
      text: `A new job of your interest has come up on the site.`,
    };
    transporter.sendMail(mailOptions, async (err, info) => {
      if (err) {
        console.log(err);
        return res.status(500).json({
          message: "Couldnt send mail"
        });
      }
    });
    return res.status(201).json({
      message: "Job Created and Notified all users",
      job
    });
  } catch (err) {
    res.status(500).json({
        message: "Couldnt create job"
    });
  }
};
